...
class TranMResProModel(Module):
    ...
    def __init__(self,config, protein_res_feature_dim,ligand_atom_feature_dim,num_props=None):
        ...
        self.projection2=nn.Linear(512, 128, bias=False)
        self.projection3 = nn.Linear(256, 128, bias=False)
        # self.fusion_model = AttentionFusion(mol_dim=512, pocket_dim=64, hidden_dim=128)
    def forward(self,batch,smiles_index,compose_feature, compose_vec, idx_ligand, idx_protein,
        ...
        pro_encoder_outputs = self.frontier_pred(
            h_compose,
        )
        pro_encoder_outputs=pro_encoder_outputs.to(device) #32，71，256
        # 线性层，将 mol_embed 从 512 维度转换为 128 维度
        # mol_linear = nn.Linear(512, 128).to(device)
        mol_embed_transformed = self.projection2(ret_embeddings)  # (32, 10, 128)

        # 线性层，将 pocket_embed 从 256 维度转换为 128 维度
        # pocket_linear = nn.Linear(256, 128).to(device)
        pocket_embed_transformed = self.projection3(pro_encoder_outputs)  # (32, x, 128)

        # 确定最终的 seqlen，可以选择最大的长度或者其他策略
        seqlen = max(mol_embed_transformed.size(1), pocket_embed_transformed.size(1))

        # 使用零填充或截断的方式调整 mol_embed 和 pocket_embed 的长度
        mol_embed_padded = torch.zeros(32, seqlen, 128).to(device)
        mol_embed_padded[:, :mol_embed_transformed.size(1), :] = mol_embed_transformed
#上面是32，71，128
        pocket_embed_padded = torch.zeros(32, seqlen, 128).to(device)
        pocket_embed_padded[:, :pocket_embed_transformed.size(1), :] = pocket_embed_transformed

        # 将两个嵌入拼接在一起
        encoder_outputs = torch.cat((mol_embed_padded, pocket_embed_padded), dim=-1)  # (32, seqlen, 256)

        # encoder_outputs = torch.cat((pro_encoder_outputs, fused_embed), dim=1)

        #这里的encoder_outputs第一个维度也应该是32
